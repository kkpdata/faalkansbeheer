...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--hr-path", type=Path, default=Path("C:/Users/SAKA/Downloads/Test_NEbe_HR"))
    parser.add_argument("--hr-calname", default="ws")
    parser.add_argument("--dir-traject", type=Path, default=Path("C:/Users/SAKA/Downloads/Test_NEbe"))
    parser.add_argument("--output-folder", type=Path, default=Path("C:/Users/SAKA/Downloads/Test_NEbe_output"))
    ...
    parser.add_argument("--wl-count", type=int, default=201)
...
def main() -> None:
    ...
    with store_ctx as store:
        for p in dir_traject.rglob("*.xlsx", case_sensitive=False):
            if p.is_file() and not p.name.startswith("~$"):
                ...
                _, fcs = eeg.get_failure_path_probabilities(water_levels=np.linspace(10.0, 13.0, 11))
    ...
    for section_name, scenarios in tqdm.tqdm(sorted_sections, desc="Sections"):
        ...
        for scen_name, (eeg, scen_prob, hr_loc) in tqdm.tqdm(scenarios.items(), leave=False, desc="Scenarios"):
            ...
            # Save tree plot
            if plot_tree:
                eeg.plot(view=False, output_path=fig_path / f"tree_scenario_{scen_name}.png", water_level=12.5)
