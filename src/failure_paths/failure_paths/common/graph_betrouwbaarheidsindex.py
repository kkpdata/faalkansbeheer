...
def _add_beta_per_uittredepunt_points(self, df_for_graph: DataFrame, mask, name: str, color: str, value: bool):
    ...
    self.fig.add_trace(
        go.Scatter(
            ...
            hovertemplate=(
                "ID: %{customdata[0]}<br>" + "Beta: %{customdata[1]:.3f}<br>" + "Metrering: %{customdata[2]}"
            ),
...
class GraphBetaValuesSingleInteractive:
    ...
    def __init__(
        ...
        df_beta_vak: DataFrame | None = None,
    ...
    def _add_beta_per_vak(self):
        ...
        for _, row in self.df_beta_vak.iterrows():
            self.fig.add_trace(
                go.Scatter(
                    ...
                    hovertemplate="ID: %{customdata[0]}<br>" + "Beta: %{customdata[1]:.3f}",
                )
            )
            first = False
        ...
        for _, row in self.df_beta_vak.loc[mask_high].iterrows():
            hovertemplate = "ID: %{customdata[0]}<br>" + "Beta: %{customdata[1]:.3f}<br>"
            if row["beta"] == np.inf:
                hovertemplate = "ID: %{customdata[0]}<br>" + "Beta: inf <br>"
            self.fig.add_trace(
                go.Scatter(
...
def _validate_input(self: GraphBetaValuesSingleInteractive):
    if self.df_beta_vak is not None:
        ...
        assert "id" in vak_columns, "Required column 'id' not found in df_beta_vak."
        assert "m_start" in vak_columns, "Required column 'm_start' not found in df_beta_vak."
        assert "m_end" in vak_columns, "Required column 'm_end' not found in df_beta_vak."
        assert "beta" in vak_columns, (
            "Required column 'beta' not found in df_beta_vak.\n"
            "If you only have pf, use scipy.stats.norm.ppf(x) to convert to beta."
        )
